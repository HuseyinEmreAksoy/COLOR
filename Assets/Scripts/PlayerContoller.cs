...
public class PlayerContoller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.GetKeyDown(KeyCode.Z)){                                                //Spawns RED bullet with Z
            Instantiate(bullets[0], gameObject.transform.position, gameObject.transform.rotation);
        }
        if(Input.GetKeyDown(KeyCode.X)){                                                //Spawns GREEN bullet with X
            Instantiate(bullets[1], gameObject.transform.position, gameObject.transform.rotation);
        }
        if(Input.GetKeyDown(KeyCode.C)){                                                //Spawns BLUE bullet with C
